...
/* Botones */
.stButton>button { background-color:#FF00AA!important; color:#fff!important; border:none!important; border-radius:4px!important; font-weight:900!important; font-family:Calibri,Arial,sans-serif!important; letter-spacing:0.4px; padding:8px 20px; }
.stButton>button:hover { background-color:#CC0088!important; }
/* Inputs */
.stTextInput>div>div>input, .stNumberInput>div>div>input { background-color:#300030!important; color:#fff!important; border:1px solid #FF00AA!important; border-radius:4px!important; font-family:Calibri,Arial,sans-serif!important; }
.stSelectbox>div>div { background-color:#300030!important; color:#fff!important; border:1px solid #FF00AA!important; }
.stDateInput>div>div>input { background-color:#300030!important; color:#fff!important; border:1px solid #FF00AA!important; }
/* Labels */
.stTextInput label,.stNumberInput label,.stSelectbox label,.stDateInput label,.stCheckbox label { color:#FF00AA!important; font-weight:900!important; font-size:12px!important; }
/* Tabs */
...
[data-testid="stMetricValue"] { color:#fff!important; font-size:16px!important; font-weight:700!important; }
/* Alerts */
...
/* Checkbox */
.stCheckbox { color:#FF00AA!important; }
/* Tabla nequi */
...
def guardar_historico(franja, fecha_reporte, reg, cli):
    get_supabase().table("historico").insert({
        "franja": franja, "fecha_reporte": fecha_reporte,
        "registro_transacciones": reg, "transacciones_clientes": cli
    }).execute()

def obtener_historico(franja):
    r = get_supabase().table("historico").select("*").eq("franja", franja).order("id", desc=True).limit(1).execute()
...
def obtener_actual(franja):
    hoy = date.today().isoformat()
    r   = get_supabase().table("registro_actual").select("*").eq("franja", franja).eq("fecha", hoy).execute()
    return r.data[0] if r.data else None

def obtener_todos_actual():
    hoy = date.today().isoformat()
    r   = get_supabase().table("registro_actual").select("*").eq("fecha", hoy).order("franja").execute()
    return r.data or []

def registrar_envio(franja, estado, respuesta):
    try:
        get_supabase().table("log_envios").insert({
...
def render_plantilla(f_sup, reg_sup, cli_sup, f_inf, reg_inf, cli_inf):
    ...
    def val_html(v, badge=""):
        num = fmt_cop(v).replace("$ ","") if v is not None else '<span style="color:#664466">Sin datos</span>'
        return f'$ &nbsp;{num} &nbsp;{badge}'

    def dir_t(d): return "por encima" if d > 0 else "por debajo" if d < 0 else "igual"

    comp = ""
    if reg_sup and reg_inf and cli_sup and cli_inf:
        comp = f"""
        <div class='banner-comp'>
          <p>El reporte muestra que actualmente en
          <b>Registro de Transacciones</b> estamos <b>{dir_t(d_reg)}</b>
          con una diferencia de <b>{fmt_cop(abs(d_reg))}</b> y en
          <b>Transacciones de Clientes</b> estamos <b>{dir_t(d_cli)}</b>
          con una diferencia de <b>{fmt_cop(abs(d_cli))}</b>.</p>
        </div>"""
    ...
    <div style='border:2px solid #fff;background:#200020;'>
      ...
      <table class='nequi-table' style='border-top:none;'>
        ...
        <tbody>
          <tr><td>REGISTRO DE TRANSACCIONES</td><td class='valor'>{val_html(reg_sup)}</td></tr>
          <tr><td>TRANSACCIONES DE CLIENTES</td><td class='valor'>{val_html(cli_sup)}</td></tr>
        </tbody>
      </table>
    </div>
    ...
    <div style='border:2px solid #fff;background:#200020;border-top:none;'>
      ...
      <table class='nequi-table' style='border-top:none;'>
        <thead><tr><th style='width:65%'>SERVICIO</th><th>VALOR</th></tr></thead>
        <tbody>
          <tr><td>REGISTRO DE TRANSACCIONES</td><td class='valor'>{val_html(reg_inf, b_reg)}</td></tr>
          <tr><td>TRANSACCIONES DE CLIENTES</td><td class='valor'>{val_html(cli_inf, b_cli)}</td></tr>
        </tbody>
      </table>
    </div>
    {comp}
...
def render_grafica(datos_act, hist_map):
    ...
    sup_reg  = [hist_map.get(franja_sup(f),{}).get("registro_transacciones") for f in franjas]
    sup_cli  = [hist_map.get(franja_sup(f),{}).get("transacciones_clientes")  for f in franjas]
    ...
    def ni(lst): return [None if v is None else int(v) for v in lst]
    ...
    fig.update_layout(
        paper_bgcolor="#200020", plot_bgcolor="#200020",
...
def enviar_teams(franja_inf, hist, act, comentario="", incluir_comentario=False):
    ...
    cuerpo = [
        {"type":"ColumnSet","style":"emphasis","bleed":True,"columns":[
            {"type":"Column","width":"auto","items":[{"type":"TextBlock","text":"· Nequi","weight":"Bolder","size":"ExtraLarge","color":"Light"}]},
            {"type":"Column","width":"stretch","verticalContentAlignment":"Center","items":[{"type":"TextBlock","text":fecha_hist,"weight":"Bolder","size":"Large","horizontalAlignment":"Center","color":"Light"}]}
        ]},
        {"type":"TextBlock","text":"TRANSACCIONALIDAD ENTRE LAS 00:00 Y LAS 20:00","weight":"Bolder","color":"Accent","horizontalAlignment":"Center","spacing":"Small"},
        ...
        {"type":"ColumnSet","style":"emphasis","spacing":"Medium","columns":[
            {"type":"Column","width":"auto","items":[{"type":"TextBlock","text":"· Nequi","weight":"Bolder","size":"ExtraLarge","color":"Light"}]},
            {"type":"Column","width":"stretch","verticalContentAlignment":"Center","items":[{"type":"TextBlock","text":fecha_hoy,"weight":"Bolder","size":"Large","horizontalAlignment":"Center","color":"Light"}]}
        ]},
        {"type":"TextBlock","text":"TRANSACCIONALIDAD ENTRE LAS 00:00 Y LAS 20:00","weight":"Bolder","color":"Accent","horizontalAlignment":"Center","spacing":"Small"},
        {"type":"Table","columns":[{"width":2},{"width":1}],"rows":[
            {"type":"TableRow","style":"emphasis","cells":[
        ...
        {"type":"TextBlock","text":f"Para la fecha {fecha_hoy}, el reporte generado a las {hora} muestra:","weight":"Bolder","spacing":"Medium","wrap":True,"color":"Light"},
        {"type":"FactSet","facts":[
    ...
    if incluir_comentario and comentario.strip():
        cuerpo.append({"type":"TextBlock","text":f"Observacion: {comentario.strip()}","wrap":True,"isSubtle":True,"spacing":"Small","color":"Light"})

    payload = {"type":"message","attachments":[{
        "contentType":"application/vnd.microsoft.card.adaptive",
        "content":{"$schema":"http://adaptivecards.io/schemas/adaptive-card.json",
                   "type":"AdaptiveCard","version":"1.4","body":cuerpo}
    }]}
...
with tab1:
    c1, c2 = st.columns(2)
    with c1:
        fh = st.selectbox("FRANJA HISTÓRICA", FRANJAS_HISTORICO, key="sel_h")
    with c2:
        fa = st.selectbox("FRANJA ACTUAL (cada 2h)", FRANJAS_ACTUAL, key="sel_a")

    hist = obtener_historico(fh)
    act  = obtener_actual(fa)
...
with tab2:
    ...
    with c1: fh2 = st.selectbox("Franja histórica", FRANJAS_HISTORICO, key="inp_fh")
    with c2: fecha_h = st.date_input("Fecha del reporte", value=date.today(), key="inp_fh_date")
    c4, c5 = st.columns(2)
    with c4: reg_h = st.number_input("Registro de Transacciones ($)", min_value=0, step=1000, key="inp_reg_h", format="%d")
    with c5: cli_h = st.number_input("Transacciones de Clientes ($)",  min_value=0, step=1000, key="inp_cli_h", format="%d")
    ...
    c6, _ = st.columns([1,3])
    with c6: fa2 = st.selectbox("Franja actual", FRANJAS_ACTUAL, key="inp_fa")
    c7, c8 = st.columns(2)
    with c7: reg_a = st.number_input("Registro de Transacciones ($)", min_value=0, step=1000, key="inp_reg_a", format="%d")
    with c8: cli_a = st.number_input("Transacciones de Clientes ($)",  min_value=0, step=1000, key="inp_cli_a", format="%d")
...
with tab3:
    ...
    hist_map  = {fk: obtener_historico(fk) for fk in FRANJAS_HISTORICO if obtener_historico(fk)}
    if not datos_act and not hist_map:
        ...
        for i, f in enumerate(FRANJAS_ACTUAL):
            ...
            hst   = hist_map.get(franja_sup(f))
            with cols[i]:
                if act_d:
                    dif = (int(act_d["registro_transacciones"])-int(hst["registro_transacciones"])) if hst and hst.get("registro_transacciones") else None
                    st.metric(f, fmt_cop(act_d["registro_transacciones"]),
                              delta=f"+{fmt_cop(dif)}" if dif and dif>0 else fmt_cop(dif) if dif else None)
                else:
                    st.metric(f, "—")
...
with tab4:
    c1, c2 = st.columns(2)
    with c1: franja_env = st.selectbox("Franja a enviar", FRANJAS_ACTUAL, key="sel_env")
    ...
    fecha_h_s   = fmt_fecha(hist_env.get("fecha_reporte")) if hist_env else "—"
...
• **Registro de Transacciones:** {est_txt(rh, ra)}
  *(actual: {fmt_cop(ra)} vs histórico: {fmt_cop(rh)} del {fecha_h_s} — franja {fk_env})*

• **Transacciones de Clientes:** {est_txt(ch, ca)}
  *(actual: {fmt_cop(ca)} vs histórico: {fmt_cop(ch)} del {fecha_h_s} — franja {fk_env})*
""")

    # Comentario OPCIONAL con checkbox
    st.markdown("---")
    ...
    if st.button("ENVIAR A TEAMS 📤"):
        if not WEBHOOK_URL:
            ...
            if ok:
                st.success(f"✓ Enviado correctamente a Teams — {hora_s}")
            else:
